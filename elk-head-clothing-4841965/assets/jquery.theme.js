...
$(document).ready(function(){
	...
	*/
	...
	Shopify.onCartUpdate = function(cart, form) {
		...
		$("#drawer .cart-container").prepend('<p class="spinner"></p>');
		$("#drawer form").remove();
		...
		if(cart.item_count > 0) {
			...
			jQ.each(cart.items, function(index, cartItem) {
                ...
                //<p>'+description+'</p>
                //<div class="two columns total desktop">'+formatMoney(cartItem.line_price)+'</div>
				drawer += '<div class="row '+first+'"><div class="eight columns description alpha"><div class="img-container"><img src="'+cartItem.image+'" alt="'+cartItem.title+'"/></div><div class="info"><h3><a href="'+cartItem.url+'">'+cartItem.title+'</a></h3><h4>'+cartItem.vendor+'</h4><p class="mobile">'+cartItem.quantity+' x '+formatMoney(cartItem.price)+'</p></div></div><div class="two columns price desktop">'+formatMoney(cartItem.price)+'</div><div class="two columns quantity desktop"><abbr>QTY</abbr><input type="text" name="updates[]" id="updates_'+cartItem.id+'" value="'+cartItem.quantity+'"/></div><div class="two columns size desktop">–</div><div class="two columns omega remove desktop"><a href="/cart/change?line='+index+'&quantity=0" class="removeLine" rel="'+cartItem.variant_id+'">&times;</a></div></div>';

				if (index == (cart.items.length-1)) {
					$("#drawer .spinner").remove();
					$("#drawer .cart-container").append('<form action="/cart" method="post">'+drawer+'<div class="actions"><input type="submit" name="checkout" value="Check Out" /></div></form>').hide().fadeIn(500);
				}
			});
    ...
    $(document).on("click", ".close-cart-drawer", function(e) {
        e.preventDefault();
        $("#drawer").slideUp()
    });
	...
	function removeLine() {
		$(".marked-delete").parent().parent().fadeOut(1000, function() {
			...
			if ($("#drawer .row").length == 0) {
				$("#drawer .cart-container").html('<div class="sixteen columns description"><p class="empty">– Your cart is empty. –</p></div>');
			}
		})
    ...
    $(document).on("click", ".icon-search", function(e) {
        e.preventDefault();
        $(".drawer").slideUp();
        $(".product-search-wrapper").slideToggle();
    });
    $(document).on("click", ".close-cart-drawer", function(e) {
        e.preventDefault();
        $(".drawer").slideUp();
  ...
  function windowResize() {
    ...
    // Make mountains unique.
    // ======================


    var randomNumber = Math.floor(Math.random() * 15) + 1;
    console.log('random number is '+randomNumber);
    $(".mountains").addClass('mountain-'+randomNumber);
